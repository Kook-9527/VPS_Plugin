...
# 流量配额精确监控脚本
# 核心逻辑：
#   1. 基于 vnstat 统计网卡总流量（接收+发送）
#   2. 按月周期监控，每月指定日期和时间自动重置
#   3. 当已用流量达到设定配额时，自动封禁指定端口
#   4. 新周期开始时自动解封端口并重置统计
#   5. 支持多端口同时监控（逗号分隔，如：55555,55556）
#   6. 支持 TG 通知：超额封禁、周期重置等事件实时推送
# 
# 使用场景：
#   - VPS 月流量限制管理
#   - 代理节点流量控制
...
# 
# 周期示例：
#   CYCLE_DAY="08"  CYCLE_TIME="22:00:14"
#   → 每月8日 22:00:14 自动重置流量并解封端口
# ============================================
