...
compatibility_old_binary() {
  ...
  if [ -d $WORK_DIR ] && ! [ -f "$WORK_DIR/np-lts" ]; then
    ...
    if [ "$DOWNLOAD_TOOL" = "curl" ]; then
      curl -sL "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${LTS_VERSION_NUM}/nodepass_${LTS_VERSION_NUM}_linux_${ARCH}.tar.gz" | tar -xz -C "$TEMP_DIR"
    else
      wget "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${LTS_VERSION_NUM}/nodepass_${LTS_VERSION_NUM}_linux_${ARCH}.tar.gz" -qO- | tar -xz -C "$TEMP_DIR"
    fi
...
upgrade_nodepass() {
  ...
  if [ "$DOWNLOAD_TOOL" = "curl" ]; then
    ...
    if [ "$HAS_STABLE_UPGRADE" = 1 ]; then
      curl -sL "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${STABLE_VERSION_NUM}/nodepass_${STABLE_VERSION_NUM}_linux_${ARCH}.tar.gz" | tar -xz -C "$TEMP_DIR"
    ...
    if [ "$HAS_DEV_UPGRADE" = 1 ]; then
      curl -sL "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${DEV_VERSION_NUM}/nodepass_${DEV_VERSION_NUM}_linux_${ARCH}.tar.gz" | tar -xz -C "$TEMP_DIR"
      if [ -f "$TEMP_DIR/nodepass" ]; then
    ...
    # 下载稳定版
    if [ "$HAS_STABLE_UPGRADE" = 1 ]; then
      wget "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${STABLE_VERSION_NUM}/nodepass_${STABLE_VERSION_NUM}_linux_${ARCH}.tar.gz" -qO- | tar -xz -C "$TEMP_DIR"
      if [ -f "$TEMP_DIR/nodepass" ]; then
        mv "$TEMP_DIR/nodepass" "$WORK_DIR/np-stb"
        chmod +x "$WORK_DIR/np-stb"
      else
    ...
    # 下载开发版
    if [ "$HAS_DEV_UPGRADE" = 1 ]; then
      wget "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${DEV_VERSION_NUM}/nodepass_${DEV_VERSION_NUM}_linux_${ARCH}.tar.gz" -qO- | tar -xz -C "$TEMP_DIR"
      if [ -f "$TEMP_DIR/nodepass" ]; then
        mv "$TEMP_DIR/nodepass" "$WORK_DIR/np-dev"
        chmod +x "$WORK_DIR/np-dev"
      else
...
install() {
  ...
  if [ "$DOWNLOAD_TOOL" = "curl" ]; then
    { curl --connect-timeout 60 --max-time 60 --retry 2 -sL "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${DEV_VERSION_NUM}/nodepass_${DEV_VERSION_NUM}_linux_${ARCH}.tar.gz" | tar -xz -C "$TEMP_DIR"; } &
    { curl --connect-timeout 60 --max-time 60 --retry 2 -sL "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${STABLE_VERSION_NUM}/nodepass_${STABLE_VERSION_NUM}_linux_${ARCH}.tar.gz" | tar -xz -C "$TEMP_DIR"; } &
    { curl --connect-timeout 60 --max-time 60 --retry 2 -sL "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${LTS_VERSION_NUM}/nodepass_${LTS_VERSION_NUM}_linux_${ARCH}.tar.gz" | tar -xz -C "$TEMP_DIR"; } &
    ...
    { wget --timeout=60 --tries=2 "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${DEV_VERSION_NUM}/nodepass_${DEV_VERSION_NUM}_linux_${ARCH}.tar.gz" -qO- | tar -xz -C "$TEMP_DIR"; } &
    { wget --timeout=60 --tries=2 "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${STABLE_VERSION_NUM}/nodepass_${STABLE_VERSION_NUM}_linux_${ARCH}.tar.gz" -qO- | tar -xz -C "$TEMP_DIR"; } &
    { wget --timeout=60 --tries=2 "${GH_PROXY}https://github.com/Kook-9527/VPS_Plugin/raw/refs/heads/main/nodepass/${LTS_VERSION_NUM}/nodepass_${LTS_VERSION_NUM}_linux_${ARCH}.tar.gz" -qO- | tar -xz -C "$TEMP_DIR"; } &
...
bash <($DOWNLOAD_COMMAND https://raw.githubusercontent.com/Kook-9527/VPS_Plugin/refs/heads/main/nodepass/np.sh) \$1
EOF
